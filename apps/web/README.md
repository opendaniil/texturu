...
`app/` содержит **только route entrypoints**.
Обязательный entrypoint: `page.tsx`.
Опциональные entrypoints: `layout.tsx`, `loading.tsx`, `error.tsx`, `not-found.tsx` и Next-специфичные экспорты (`generateMetadata`, `generateStaticParams` и т.п.).

Каждый существующий entrypoint — **тонкая обёртка**, которая импортирует реальный UI/логику из `src/pages/` или `src/widgets/`.
...
```tsx
// app/article/[slug]/layout.tsx — тонкий entrypoint (опционально)
...
Когда добавлять опциональные entrypoints:

* `layout.tsx` — нужен локальный каркас сегмента.
* `loading.tsx` — нужен локальный fallback во время загрузки.
* `error.tsx` — нужен локальный recovery UI (`reset()`).
...
## Стили: Tailwind, лейаут-слои, готовые секции

> Единые ширины, поля, ритм. Код, нарушающий роли слоёв, — техдолг.

---

### Роли слоёв

| Слой | Отвечает за | Не трогает |
|------|------------|------------|
| **PageShell** | `min-h-dvh`, фон страницы, header/footer | `px-*`, `py-*`, `max-w-*` |
| **Section** | Вертикальный ритм (`py-*`), фон секции | Ширину, горизонтальные поля |
| **Container** | `mx-auto max-w-* px-*` | Вертикальные отступы, фон |
| **Content layout** | `grid`/`flex`, `gap`, `md:grid-cols-*` | Ширину страницы |

Секция не решает ширину — это делает только Container.

---

### Токены контейнера

Три размера, больше не добавлять без явной причины:

```ts
// shared/ui/container.ts
...
### Интеграция готовых секций

Готовый блок приходит со своей обвязкой. Мы её снимаем:

1. **Удалить** из блока: `container`, `max-w-*`, `mx-auto`, `px-*`, обычно `py-*`.
2. **Обернуть** стандартной структурой:
...
3. Внутри оставить только контент и раскладку.

Допустимые `max-w-*` внутри секции: `max-w-prose` / `max-w-2xl` для текстовых блоков, узкая форма/карточка. Но не как контейнер страницы.

---

### Фон до края

Фон/декор — на Section. Контент — в Container.
...
<section className="relative py-24 bg-slate-900">
  ...
  <div className={cn(container.default, 'relative')}>
    {/* контент */}
  </div>
</section>
```

Фон внутри контейнера обрежется по `max-w` — не делать.

---

### Антипаттерны

Внутри секций запрещены по умолчанию:

* `container`, `mx-auto`, `max-w-*` — дублирует Container
* повторные `px-*` — двойные поля
* `w-screen` / `-mx-*` — ломает каркас
